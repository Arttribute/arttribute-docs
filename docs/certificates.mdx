...
Creating a certificate requires an access token to identify the user on whose behalf the application is working. Ensure the user is registered on Arttribute with a wallet. See the process for [Generating access tokens](/guide/generate-keys#generating-user-access-tokens).
...
1. **Checking for Permissions**: Some items or collections may require explicit permission from the owner before they can be used or referenced. In such cases, you need to [Send a permission request](/guide/permission-request#sending-perssion-request) to the owner. Once the owner grants permission, you can move on to the next steps.

2. **Making Payments**: If the item or collection has a price tag, you'll need to make the necessary payment. Ensure the payment is successful before proceeding. See the process for [Making payments for items and collections](/guide/payments#making-payments).
