...
Creating a certificate requires an access token to identify the user on whose behalf the application is working. Ensure the user is registered on Arttribute with a wallet. See the process for [Generating access tokens](/generate-access-token#generating-user-access-tokens).
...
axios
  .request({
    method: "post",
    maxBodyLength: Infinity,
    url: "https://dev.api.arttribute.io/v1/certificates/mint/:certificateId", // Replace with actual API URL and certificate ID
    headers: {
      "Content-Type": "application/json",
      Authorization: "Bearer your_access_token",
    },
...
```bash title="Making a POST request with cURL"
curl --location --request POST 'https://dev.api.arttribute.io/v1/certificates/mint/:certificateId' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer your_access_token' \
--data '{
  "message": "Your custom message, if any.",
