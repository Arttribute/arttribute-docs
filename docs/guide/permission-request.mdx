...
To create a permission request, you first need to be authenticated as a registered Arttribute user. This ensures that both the sender and receiver identities are verified within the platform. The authentication can be done using [Arttribute's authentication mechanism](/guide/auth).
