...
Before you can integrate this functionality into your application, you'll need an API key. This key is essential for authenticating your requests and gaining access to the Arttribute's features. To obtain an API key, follow the steps in the [Generating Keys section](/generate-access-token)

### Step 2: Obtain an access token

The collections resource of the Arttribute API requires an access token to identify the user on whose behalf the application is creating or interacting with art collections. This user should be an Arttribute user and should have a registered wallet. See how to [Generating access tokens](/generate-access-token#generating-user-access-tokens). If an access token is not provided, the collection will be added under the account of the user who generated the API Key provided
...
axios
  .request({
    method: "post",
    url: "https://dev.api.arttribute.io/v1/collections", //TODO: Change this to API base url variable
    headers: {
      "x-api-key": "17eoF8...",
...
curl --location 'https://dev.api.arttribute.io/v1/collections' \
...
let config = {
  ...
  url: "https://dev.api.arttribute.io/v1/collections/:id", //TODO: Change to url var
};
...
curl --location 'https://dev.api.arttribute.io/v1/collections/:id'
```
...
axios
  .request({
    method: "get",
    maxBodyLength: Infinity,
    url: "https://dev.api.arttribute.io/v1/collections",
  })
  .then((response) => {
    console.log(JSON.stringify(response.data));
  })
...
axios
  .request({
    method: "patch",
    maxBodyLength: Infinity,
    url: "https://dev.api.arttribute.io/v1/collections/:id",
    headers: {
      "Content-Type": "application/json",
      Authorization: "Bearer eyJhbGciOiJIU...",
    },
...
curl --location --request PATCH 'https://dev.api.arttribute.io/v1/collections/:id' \
--header 'Content-Type: application/json' \
...
axios
  .request({
    method: "patch",
    maxBodyLength: Infinity,
    url: "https://dev.api.arttribute.io/v1/collections/:id/items",
    headers: {
      "Content-Type": "application/json",
      Authorization: "Bearer eyJhbGciOiJIU...",
    },
...
```bash title="Making a PATCH request with cURL"
curl --location --request PATCH 'https://dev.api.arttribute.io/v1/collections/:id/items' \
--header 'Content-Type: application/json' \
--header 'Authorization: Bearer eyJhbGciOiJIUzI1N...' \
--data '{
    "itemId": "05d7cd..."
...
axios
  .request({
    method: "delete",
    maxBodyLength: Infinity,
    url: "https://dev.api.arttribute.io/v1/collections/:id/items/:itemId",
    headers: {
      Authorization: "Bearer eyJhbGciOiJIU...",
    },
    data: JSON.stringify(data),
...
curl --location --request DELETE 'https://dev.api.arttribute.io/v1/collections/:id/items/:itemId' \
--header 'Authorization: Bearer eyJhbGciOiJIUzI1N...' \
```
...
axios
  .request({
    method: "delete",
    maxBodyLength: Infinity,
    url: "https://dev.api.arttribute.io/v1/collections/:id", //TODO: Change to variable
    headers: {
...
```bash title="Making a DELETE request with cURL"
curl --location --request DELETE 'https://dev.api.arttribute.io/v1/collections/:id' \
--header 'Authorization: Bearer eyJhbGciOiJIU...'
```
