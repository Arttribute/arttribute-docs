...
| `license`      | Object conataining licensing information of the item. See our [list of supported licenses](/licenses) to check what values this field can take |
| `price`        | Price information of the item. See our [supported currencies](/payments#supported-currencies) to check what values this field can take         |
...
- **Data URL Format:** You can use a [data URL](https://developer.mozilla.org/en-US/docs/Web/HTTP/Basics_of_HTTP/Data_URLs). A data URL is a base64-encoded string prepended with the encoding type (base64) and MIME type of the file. The MIME type is included within the data URL itself, making it a self-contained format. This is a convenient way to provide the file and its associated information in one string and is the preferred option. This can be done through a libraries of the language of your choice or, alternatively, by using our [File service](/file-service).
...
axios
  .request({
    ...
    url: "https://dev.api.arttribute.io/items",
    headers: {
      "x-api-key": "17eoF8...",
...
curl --location 'https://dev.api.arttribute.io/items' \
...
let config = {
  method: "get",
  maxBodyLength: Infinity,
  url: "https://dev.api.arttribute.io/items/:id",
  headers: {
...
curl --location 'https://dev.api.arttribute.io/items/:id' \
...
axios
  .request({
    method: "get",
    maxBodyLength: Infinity,
    url: "https://dev.api.arttribute.io/items?tags=cgi,art&source=instagram",
    headers: {
...
```bash title="Making a GET request with cURL"
curl --location 'https://dev.api.arttribute.io/items?tags=cgi%2Cart&source=instagram' \
...
curl --location --request PATCH 'https://dev.api.arttribute.io/items/:id' \
...
curl --location --request DELETE 'https://dev.api.arttribute.io/items/:id' \
--header 'Authorization: Bearer eyJhbGciOiJIU...'
```
